...
- Output file has no metadata (aside from original file size, which is stored in the fisrt 8 bytes of an encrypted file), not even hash of the source file, thus, while decrypting, the program can't detect if the password is wrong, and if an incorrect password is provided you'll just get garbage data output

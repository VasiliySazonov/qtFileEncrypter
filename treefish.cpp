...
void threefish_set_key(threefish_key_t *key, uint64_t *key_data, uint64_t *key_tweak)
{
    ...
    key->tweak[0] = key_tweak[0];
    key->tweak[1] = key_tweak[1];
    key->tweak[2] = key_tweak[0] ^ key_tweak[1];

    for (int i = 0; i < KEY_SIZE_WORDS; i++)
    {
        key->key[i] = key_data[i];
        parity ^= key_data[i];
    }

    key->key[KEY_SIZE_WORDS] = parity;
}
...
void threefish_encrypt(const threefish_key_t *key, const uint64_t *input, uint64_t *output)
{
    ...
    uint64_t k0 = key->key[0], k1 = key->key[1],
             k2 = key->key[2], k3 = key->key[3],
             k4 = key->key[4];
    uint64_t t0 = key->tweak[0], t1 = key->tweak[1],
             t2 = key->tweak[2];
